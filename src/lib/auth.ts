...
export async function login(email: string, password: string) {
  ...
  // 쿠키에 세션 토큰 저장은 서버 액션에서 처리
  return { success: true, user, token, expires }
}
...
// 현재 로그인한 사용자 가져오기 (서버 액션에서 토큰을 전달받음)
...
// 인증이 필요한 페이지에서 사용할 함수 (서버 액션에서 토큰을 전달받음)
export async function requireAuth(token: string | undefined, redirectUrl: string) {
  const user = await getCurrentUser(token)

  if (!user) {
    redirect(`/login?callbackUrl=${encodeURIComponent(redirectUrl)}`)
  }
